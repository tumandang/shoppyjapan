...
import { ChevronDown,ChevronUp,FileImage,Package,MapPin,User,Phone,Mail,Calendar,Truck,CreditCard} from "lucide-react";
        ...
        <div className="bg-gray-50 rounded-lg p-5 border border-gray-200">
          ...
          <div className="space-y-2">
            <p className="text-sm text-gray-900 font-medium">
              {user.customer.address.address1} {user.customer.address.address2} {user.customer.address.address3}
            </p>
            <p className="text-sm text-gray-600">
              {user.customer.address.postcode} {user.customer.address.city} {user.customer.address.state}
        ...
        <div className="bg-gray-50 rounded-lg p-5 border border-gray-200">
          ...
          <div className="space-y-3">
            ...
            <div className="border-t border-gray-300 pt-3 mt-3">
              <div className="flex items-center justify-between">
                <span className="text-sm font-semibold text-gray-900">Total (JPN)</span>
            ...
            <div className="border-t border-gray-300 pt-3 mt-3">
              <div className="flex items-center justify-between">
                <span className="text-sm font-semibold text-gray-900">Total (MAS)</span>
                <span className="text-lg font-bold text-gray-900">
                  RM {order.amount_myr}
                </span>
              </div>
            ...
            <div className="bg-white rounded-lg p-3 border border-gray-200 mt-4">
              ...
              <div className="flex items-center justify-between text-sm mt-2">
                ...
                <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium capitalize ${
                  order.request?.status === 'paid' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'
                }`}>
...
const renderOverview = (order) => {
  return (
    <div className="bg-blue-50 border border-blue-200 rounded-xl p-6 space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div>
          <p className="text-xs text-blue-700">Order Status</p>
      ...
      <p className="text-sm text-blue-800 leading-relaxed">
        Your item has been successfully purchased and delivered to our Japan warehouse.
        Our team is currently measuring the package and preparing the international
        shipping cost.
      </p>

      <p className="text-sm text-blue-800">
        No action is required from you at this moment.
        You will be notified once the shipping price is ready for payment.
      </p>
  ...
  const renderTracking = (order) => {
    return (
      <div className="text-center py-12">
        ...
        <h3 className="text-sm font-medium text-gray-900 mb-2">Order Tracking</h3>
        <p className="text-xs text-gray-500 max-w-md mx-auto">
          Track your shipment and delivery status here
        </p>
      </div>
    );
  };
    ...
    return (
      <div className="mx-auto max-w-md">
        <div className="bg-red-50 border border-red-200 rounded-xl p-6 text-center">
          ...
          <p className="text-sm font-medium text-red-900 mb-1">Error loading orders</p>
            ...
            <div className="flex items-center justify-between gap-4">
              ...
              <div className="flex items-start gap-4 flex-1">
                ...
                <div className="grid grid-cols-3 gap-6 flex-1">
                  <div>
                    ...
                    <p className="text-sm font-semibold text-gray-900">#SHP{order.id}</p>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium border capitalize ${getStatusColor(order.request?.status)}`}>
              ...
              <div className="flex items-center gap-3">
                <div className="text-right">
                  ...
                  <p className="text-sm font-medium text-gray-900">{order.request.market_name}</p>
                </div>
                <button
            ...
            <div className="p-5">
              ...
              <div className="flex gap-1 bg-gray-100 p-1 rounded-lg mb-6">
                {tabOptions.map((tab) => (
                  <button
                    key={tab}
                    onClick={() => setActiveTab((prev) => ({ ...prev, [order.id]: tab }))}
              ...
              <div className="mt-4">
                {(activeTab[order.id] || "Overview") === "Overview" && renderOverview(order)}
                {(activeTab[order.id] || "Overview") === "Order Details" && renderOrderDetails(order)}
                {(activeTab[order.id] || "Overview") === "Track" && renderTracking(order)}
              </div>
            </div>
          )}
        </div>
